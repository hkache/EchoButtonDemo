...
const RollCall = {
    NewSession: function(handlerInput) {
        ...
        const ctx = handlerInput.attributesManager.getRequestAttributes();
         ...
         ctx.outputSpeech = ["Hello all. Welcome to Dreamforce."];
         ctx.outputSpeech.push("We are going to have some fun with IoT enabled buttons.");
         ctx.outputSpeech.push("Holger please go ahead and activate both buttons now.");
    ...
    HandleFirstButtonCheckIn: function(handlerInput) {
        ...
        MuleSoft.PostToListener(ctx.gameInputEvents, "green", function(status) {
                console.log("MuleSoft.PostToListener status received: " + status) ;
                console.log("<<<<<<<<<<<<<<<<< MuleSoft.PostToListener CALLBACK ***********");
        });
        ...
        if (sessionAttributes.buttonCount === 0) {                        
            ...
            ctx.outputSpeech = ['I registered button 1.'];
            ctx.outputSpeech.push(Settings.WAITING_AUDIO);
    ...
    HandleSecondButtonCheckIn: function(handlerInput) {
        ...
        if (sessionAttributes.buttonCount == 0) {
            ...
            ctx.outputSpeech.push("I registered button 2");
            ctx.outputSpeech.push("<break time='1s'/>");
        ...
        ctx.outputSpeech.push("Now let's trigger some button events.");
